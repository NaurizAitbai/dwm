...
attachclients(Monitor *m) {
	...
	for (c = cl->clients; c; c = c->next) {
		if (ISVISIBLE(c, m)) {
			 ...
			 * displayed on other monitors, remove these tags */
			if (c->tags & utags) {
				if (c->tags & ~utags) {
					c->tags &= ~utags;
				} else {
...
configurenotify(XEvent *e)
{
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				for (c = cl->clients; c; c = c->next)
					if (c->isfullscreen && ISVISIBLE(c, m))
...
createmon(void)
{
	...
	for (m = mons; m; m = m->next)
		i++;
	if (i >= LENGTH(tags)) {
		fprintf(stderr, "dwm: fialed to add monitor, number of tags exceeded\n");
		return NULL;
	}

	/* find the first tag that is not in use */
	for (i = 0; i < LENGTH(tags); i++) {
		for (m = mons; m && (m->tagset[m->seltags] & (1 << i)) == 0; m = m->next);
		if (!m)
			break;
	}

	/* reassign all tags to monitors since there is currently no
	 * free tag for the new monitor */
	if (i >= LENGTH(tags)) {
		i = 0;
		for (m = mons; m; m = m->next) {
			m->seltags ^= 1;
...
detachstack(Client *c)
{
	...
	*tc = c->snext;
	...
	if (c == c->mon->sel) {
		for (t = cl->stack; t && !ISVISIBLE(t, t->mon); t = t->snext);
		c->mon->sel = t;
	}
}
...
tag(const Arg *arg)
{
	...
	if (selmon->sel && newtags) {
		...
		if(newmon && newmon != selmon) {
			/* if tag is visible on another monitor, move
			 * client to the new monitor */
			selmon->sel->mon = newmon;
			arrange(newmon);
		}
		focus(NULL);
		arrange(selmon);
...
toggleview(const Arg *arg)
{
	...
	if (newtagset) {
		...
		for (m = mons; m; m = m->next)
			if (m != selmon && (m->tagset[m->seltags] & newtagset))
				return;
...
{
	Monitor *m, *tm;
	unsigned int oldtagset = selmon->tagset[selmon->seltags];
	unsigned int newtagset = selmon->tagset[selmon->seltags ^ 1];

	if ((arg->ui & TAGMASK) == selmon->tagset[selmon->seltags])
		return;

	/* swap tags when tyring to display a tag from another monitor */
	if (arg->ui & TAGMASK)
		newtagset = arg->ui & TAGMASK;
	for (m = mons; m; m = m->next) {
		if (m != selmon && (m->tagset[m->seltags] & newtagset)) {
			/* prevent displaying all tags (MODKEY-0) when
...
zoom(const Arg *arg)
{
	...
	|| (selmon->sel && selmon->sel->isfloating))
		return;
	if (c == nexttiled(cl->clients, c->mon))
